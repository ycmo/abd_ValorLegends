...
class TestArcaneForgeAscendTask(unittest.TestCase):
    ...
    def test_run_success_loop_and_exit(self, mock_read_image, mock_matchTemplate, mock_ocr, mock_sleep):
        ...
        self.ascend_checks = [self.fake_ascend_res, self.fake_ascend_res, self.fake_ascend_res, None, None]
        self.auto_add_checks = [None, self.fake_auto_add_res, self.fake_auto_add_res]
        def mock_match_template(screen, tpl_path):
            name = tpl_path.name
            if name == "升星.png":
                return self.ascend_checks.pop(0) if self.ascend_checks else None
            elif name == "自動添加.png":
                return self.auto_add_checks.pop(0) if self.auto_add_checks else None
            elif name == "升星成功.png":
        ...
        self.mock_ctrl.tap.assert_has_calls([
            call(10, 10),   # try ascend
            call(572, 148), # fallback slot
            call(10, 10),   # retry ascend
            call(20, 20),   # auto add
            call(10, 10),   # confirm ascend
            call(480, 50),  # close success popup
            call(40, 40),   # lock (because 2 max stats)
            call(50, 50),   # back
            call(572, 148)  # 2nd round awaken slot
        ])
    ...
    def test_run_breaks_on_low_dust(self, mock_ocr, mock_sleep):
        ...
        self.mock_vm.match_template.side_effect = lambda s, t: self.fake_ascend_res if t.name == "升星.png" else self.fake_auto_add_res if t.name == "自動添加.png" else None

        result = self.task.run()

        self.assertTrue(result)

        # 驗證 tap 順序: 只點擊升星，然後因為紫粉不足結束
        self.mock_ctrl.tap.assert_has_calls([
            call(10, 10),
        ])
        self.assertEqual(self.mock_ctrl.tap.call_count, 1)
