...
- Shared current-account state lives at `AwayFromKeyboard/state/current_account.json`.
- Use `src.account_state.write_current_account(account, source=...)` and `read_current_account()`. Do not create per-task account-state files.
- State format: `{"account": "311", "source": "switch_account.switched", "updated_at": "2026-07-01T16:45:00+08:00"}`.
- `switch_account/switch_account.py` writes the state when it detects the current account, when the target account is already current, and after a successful switch. It also supports `python -m switch_account.switch_account detect` to refresh the state without switching accounts.
- AFK loops write the same state when they detect the current account and after successful account switches. Tasks should read this file and fall back to `default` if missing/stale/unknown.
...
- Debug-stage rule: keep the normal action/screenshot/recognition loop, but persist only reviewable evidence. Use `--debug-actions` or `VL_DEBUG_ACTIONS=1`; action before/after screenshots for `tap`, `swipe`, and `keyevent` are saved under `captures/action_debug/`.
- Do not save bare polling screenshots from every `controller.screenshot()` call. Recognition/debug screenshots should be explicit `save_annotated_debug()` outputs and must include the template file name(s), ROI/box, threshold or best confidence, and why the route stopped or continued.
- Debug log messages for recognition loops should name the template being matched and the confidence, e.g. `template=training_button.png best_confidence=0.731`.
- Important no-op decisions must be reviewable. If automation skips an action because it thinks the desired state is already active, log/save the matched state template and confidence.
- For competing templates in one ROI, such as plan 1 vs plan 2 or done/claim/go statuses, compare all candidate confidences and require a margin. Do not let the first template over threshold win silently.
...
- Summon page recognition uses only `advanced_contract_label.png`, checked about once per second for up to 10 seconds. The old Chinese EasyOCR fallback was removed to avoid loading the large Chinese OCR model during routine AFK runs.
...
- Recurring/AFK Midas must enter through `AwayFromKeyboard/route_screenshots/點金手`, not through Daily Tasks. After the AFK route opens the dialog, use `run-current-scene-task midas`; never use `run-task midas` in the AFK loop because its fallback navigation can incorrectly enter Daily Tasks when the dialog was not recognized.
- `AwayFromKeyboard/loop_toggle_midas.py --auto` directly executes `MidasTask.execute_auto()` after the AFK route. A missing Midas dialog is an error and must not fall back to Daily Tasks.
- Both auto and legacy/non-auto modes in `loop_toggle_midas.py` directly use `RouteNavigator("點金手")` plus `MidasTask`; this script must not read `afk_tasks.ini`. The INI remains for generic `loop_afk.py`/Router flows only.
- If the Midas dialog is missing after the route clicks the coin `+`, both loop modes check the shared Router gift-pack blocker. A confirmed blocker is closed, the UI is recovered to main, and the Midas entry route is retried up to three times. Each failed dialog check is always saved under `captures/failures/midas/`, independently of `--debug-actions`.
- Midas auto scheduling: startup runs a special initial round first: run Midas once on the current visible account, then switch to `em3`, run Midas again, and read the first long-sleep cooldown. After that sleep, normal rounds start at `em3`, traverse the configured two/all-account order once, then return to `em3` for one final Midas attempt and cooldown read. Intermediate OCR failures mean "do not wait" and continue to the next account. Only a failed final `em3` OCR read triggers the two-hour fallback sleep. Valid final cooldowns are reduced by four minutes to leave wake/login recovery time. Any account with a valid cooldown of at most five minutes waits in place and retries.
- Full Midas auto rounds use the fixed order `em3 -> 311 -> tiger -> 14 -> em3`, minimizing Google/Email transitions. Do not derive this order from or rewrite `accounts.json`; use that file only to validate/load account data.
...
- 2026-07-01: Arena now reads `config/arena.jsonc`. Default `daily` mode preserves the existing target of at least 8 selected opponents. `tickets_20` mode can be enabled per command with `--arena-mode tickets_20`, or by changing the config; it reads the top-center ticket count on the Arena main screen and stops when tickets are `<=20`.
- Arena refresh is shared across modes, configured at the top level of `config/arena.jsonc`. When every opponent is filtered out by the safe power threshold, it first unchecks all over-threshold selected opponents; only when the selected count is then `0` does it tap yellow `刷新`. The template is `assets/tasks/arena/refresh_button.png`, cropped from `manual_screenshots/競技場/003_選擇對手.png`.
- `config/arena.jsonc` supports per-account `max_power_k` overrides. Arena reads `AwayFromKeyboard/state/current_account.json` through `src.account_state` and falls back to `accounts.default` when the current account is missing or not configured.
...
## Advanced Arena Notes

- `advanced_arena` is an independent debug-stage task for `高階競技場`, implemented in `src/tasks/advanced_arena.py`.
- First version starts from the already-open Advanced Arena screen like `manual_screenshots/高階競技場/006.png`; it is not connected to Daily Tasks or run-all.
- Policy: use only free challenge tickets, up to three fights per run. If the challenge dialog has no `免費` button, close the dialog and stop.
- Before fighting, it reads the green season countdown text. If the parsed day count is `0`, it taps the top-left back arrow and reports failed so AFK stops: the last season day should not be fought.
- The season countdown OCR uses a green-text preprocessing pass over the `9天18小時` ROI and reads only digits. On the current `006.png`, the OCR output is noisy (`9718415`) but the first digit correctly parses as `9`.
- Command: `.\.venv-codex\Scripts\python.exe -m src.main --debug --debug-actions run-current-scene-task advanced_arena`.
