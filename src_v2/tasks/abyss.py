...
class AbyssTask(BaseTask):
    ...
    def _execute_and_return(self, started: float) -> TaskRunResult:
        # Abyss 不需要 return to daily，直接結束
    ...
    def _is_main_done_zero(self) -> bool:
        ...
        match = self.context.matcher.match_template(
            screen,
            self._asset_path("main_done_zero.png"),
    ...
    def _scan_rental_view(self, screen, scan_index: int, debug_dir: Path) -> list[AbyssRentalRow]:
        rent_matches = self.context.matcher.match_template_all(
            screen,
            self._asset_path("rent_button.png"),
            threshold=0.78,
        ...
        rented_matches = self.context.matcher.match_template_all(
            screen,
            self._asset_path("rented_button.png"),
            threshold=0.78,
            roi=self.RENT_BUTTON_ROI,
            check_brightness=False,
            min_center_distance=55,
    ...
    def _is_dragon_rental_row(self, screen, row_y: int) -> bool:
        ...
        match = self.context.matcher.match_template(
            screen,
            self._asset_path("dragon_hero.png"),
    ...
    def _tap_rental_entry(self) -> None:
        ...
        while time.time() < deadline:
            ...
            match = self.context.matcher.match_template(
                screen,
                self._asset_path("rental_entry_button.png"),
    ...
    def _wait_for_rental_view(self) -> None:
        ...
        while time.time() < deadline:
            ...
            match = self.context.matcher.match_template(
                screen,
                self._asset_path("forest_tab.png"),
                threshold=0.40,
    ...
    def _wait_for_template(
        ...
        *,
        ...
        while time.time() < deadline:
            ...
            if match is not None:
                return match
            probe = self.context.matcher.best_template_match(screen, self._asset_path(asset_name), roi=roi)
    ...
    def _tap_rented_hero(self) -> None:
        ...
        if available is None:
            probe = self.context.matcher.best_template_match(
                screen,
                self._asset_path("rented_hero_available.png"),
        ...
        probe = self.context.matcher.best_template_match(
            screen,
            self._asset_path("rented_hero_selected.png"),
            roi=self.RENTED_HERO_ROI,
        )
        best_text = "none" if probe is None else f"{probe.confidence:.3f}"
        self._save_abyss_debug(
    ...
    def _close_artifact_dialog_if_open(self) -> None:
        ...
        artifact_tab = self.context.matcher.match_template(
            screen,
            self._asset_path("artifact_tab_2.png"),
            threshold=0.78,
    ...
    def _tap_confirm_if_visible(self) -> None:
        ...
        match = self.context.matcher.match_template(
            screen,
            self._asset_path("confirm_button.png"),
            threshold=0.58,
    ...
    def _tap_start_training(self) -> None:
        ...
        match = self.context.matcher.match_template(
            screen,
            self._asset_path("start_training_button.png"),
            threshold=0.78,
            roi=(820, 420, 130, 110),
        )
        if match is None:
    ...
    def _wait_skip_and_keep_result(self) -> None:
        ...
        while time.time() < deadline:
            ...
            initial_done = self.context.matcher.match_template(
                screen,
                self._asset_path("initial_done_zero.png"),
            ...
            if initial_done is not None:
                exit_match = self.context.matcher.match_template(
                    screen,
                    self._asset_path("exit_button.png"),
                    threshold=0.80,
            ...
            keep_result = self.context.matcher.match_template(
                screen,
                self._asset_path("keep_result_button.png"),
                threshold=0.78,
            ...
            skip = self.context.matcher.match_template(
                screen,
                self._asset_path("skip_button.png"),
                threshold=0.78,
                roi=self.SKIP_ROI,
            )
            if skip is not None:
    ...
    def _run_post_battle_result_sequence(self) -> None:
        ...
        while time.time() < deadline:
            screen = self.context.controller.screenshot()
                ...
                screen,
                self._asset_path("final_done_zero.png"),
        ...
        *,
        ...
        for attempt in range(1, self.EXIT_RESULT_MAX_TAPS + 1):
            ...
            after_done = self.context.matcher.match_template(
                after,
                self._asset_path(done_asset_name),
