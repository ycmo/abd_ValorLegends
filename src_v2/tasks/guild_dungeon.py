...
class GuildDungeonTask(BaseTask):
    ...
    def _daily_attempts_exhausted(self, screen) -> bool:
        best_match = self.context.matcher.match_template(
            screen,
            self._asset_path("remaining_attempts_zero_anchor.png"),
            threshold=0.0,
    ...
    def _is_plain_map_screen(self, screen) -> bool:
        ...
        return self.context.matcher.match_template(
            screen,
            self._asset_path("outpost_close_button.png"),
    ...
    def _wait_for_map_screen(self, label: str, timeout_seconds: float = 30.0) -> None:
        # Retained custom loop because it waits for multiple conditions (map title or outpost close)
    ...
    def _match_map_title_on_screen(self, screen) -> Optional[MatchResult]:
        return self.context.matcher.match_template(
            screen,
            self._asset_path("map_title_anchor.png"),
            threshold=0.86,
            roi=(0, 0, 260, 110),
        )
    ...
    def _find_challenge_probe(
        ...
        *,
        ...
        remaining_matches = self.context.matcher.match_template_all(
            screen,
            self._asset_path("remaining_attempt_anchor.png"),
        ...
        challenge_matches = self.context.matcher.match_template_all(
            screen,
            self._asset_path("challenge_button.png"),
    ...
    def _start_battle_from_ready_screen(self) -> None:
        # Retained custom loop because BATTLE_READY_ASSET is a cross-task path
    ...
    def _wait_for_battle_continue(self) -> None:
        ...
        if match is None:
            raise TaskFailedError("Guild dungeon battle continue button not found after 150s")
        self.context.controller.annotate_next_tap_debug(
            lines=[
    ...
    def _pre_return_hook(self) -> None:
        self._wait_for_returnable_screen_after_battle()
        self._close_outpost_for_return()
        self._wait_for_map_or_daily_after_close()

    def _close_outpost_for_return(self) -> bool:
        screen = self.context.controller.screenshot()
        close_match = self._match_outpost_close_on_screen(screen)
        if close_match is None:
            return False
    ...
    def _wait_for_map_or_daily_after_close(self, timeout_seconds: float = 8.0) -> bool:
        # Retained custom loop because it waits for multiple conditions (map title, daily tasks scene, or outpost close missing)
    ...
    def _wait_for_returnable_screen_after_battle(self) -> bool:
        # Retained custom loop because it waits for multiple conditions and taps continue button if seen
        if not hasattr(self.context, "detector"):
            return True
        ...
        while time.time() <= deadline:
            ...
            continue_match = self.context.matcher.match_template(
                screen,
                self._asset_path("continue_button.png"),
                threshold=self.CONTINUE_BUTTON_THRESHOLD,
                roi=self.CONTINUE_ROI,
            )
