...
- Known temporary blockers are handled by the shared `BlockerHandler` and task-level helpers in `BaseTask`; do not reimplement per-task reward-overlay detection unless the shared layer cannot model the screen safely.
- Shared reward/got-overlay detection uses a reviewed template pool under `assets/shared/got/*.png`. `BlockerHandler.match_reward_acquired()` checks this pool first in the central title ROI `(250, 80, 520, 170)` at threshold `0.72`, supports alpha-masked PNGs, then falls back to `assets/shared/reward_acquired_title.png`, then to the conservative cyan-title color heuristic.
- When adding new got/reward templates, copy only reviewed title/overlay crops into `assets/shared/got/`. Avoid item cards, bonus icons, reward option cards, and task-specific content that can appear without a blocking overlay.
- Before any task recognition loop, tab scan, or scroll fallback that can be blocked by an overlay, call `BaseTask.handle_known_blocker_before_scan(screen)`. If it returns `True`, discard that screenshot and restart the loop. This is required even if `BlockerHandler.handle_known_blocker()` reports that the overlay did not fully clear; a visibly blocked screen must not be used for matching or swiping.
