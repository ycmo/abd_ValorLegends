...
file_path = Path("E:/antigravity/adb_vl/switch_account/switch_account.py")
content = file_path.read_text(encoding="utf-8")

# Chunk 1: select_server
target1 = """    # 尋找所有該伺服器的模板圖片
...
# Chunk 2: switch_account flow
target2 = """    print("📸 開始切換帳號流程...")
...
# Fix the early account verification skip for toggle
# Actually, the original code had:
early_check = """    if account_name not in ACCOUNTS:
        print(f"錯誤：找不到帳號 '{account_name}'。支援的帳號有：{', '.join(ACCOUNTS.keys())}")
        return False

    account_info = ACCOUNTS[account_name]"""

early_replace = """    if account_name != "toggle" and account_name not in ACCOUNTS:
...
content = content.replace(early_check, early_replace)
content = content.replace(target2, replace2)

# Chunk 3: 006 matching
target3 = """        # 2. 檢查是否在進入遊戲的畫面 (006)
...
replace3 = """        # 2. 檢查是否在進入遊戲的畫面 (改為判斷靜態的使用者條款，避免動畫干擾)
...
# Chunk 4: parser
target4 = """    parser.add_argument("account", choices=ACCOUNTS.keys(), help="要切換的帳號 (google, 14, tiger)")"""
replace4 = """    parser.add_argument("account", choices=list(ACCOUNTS.keys()) + ["toggle"], help="要切換的帳號 (google, 14, tiger, toggle)")"""

content = content.replace(target4, replace4)

file_path.write_text(content, encoding="utf-8")
print("Successfully updated switch_account.py")
